...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[cconv]")==0
            || strcmp(type, "[c_convolutional]")==0) return C_CONVOLUTIONAL;
...
surgery_params parse_surgery_params(list *options) 
{
	surgery_params result;
	result.surg_c_rate = option_find_float(options, "c_rate", 2.f);
    result.surg_gamma = option_find_float(options, "gamma", 0.0001f);
    result.surg_power = option_find_float(options, "power", 1.f);
    result.surg_mu = calloc(1, sizeof(float));
    result.surg_stdev = calloc(1, sizeof(float));
    
    *result.surg_mu = 0.0f;
    *result.surg_stdev = 0.0f;
    
    result.stop_iter_ = option_find_int(options, "stop_iter", 14000);
    return result;	
}

c_convolutional_layer parse_c_convolutional(list *options, size_params params)
{
	int n = option_find_int(options, "filters",1);
    int size = option_find_int(options, "size",1);
    int stride = option_find_int(options, "stride",1);
    ...
    c_convolutional_layer layer = make_c_convolutional_layer(batch,h,w,c,n,size,stride,padding,activation, batch_normalize, binary, xnor, params.net.adam);
    surgery_params surgery = parse_surgery_params(options);

    layer.surgery = surgery;
    layer.flipped = option_find_int_quiet(options, "flipped", 0);
    layer.dot = option_find_float_quiet(options, "dot", 0);
    if(params.net.adam){
        layer.B1 = params.net.B1;
...
void parse_net_options(list *options, network *net)
{
    ...
    net->start_batch_idx = option_find_int(options, "start_batch_idx", 0);
...
network parse_network_cfg(char *filename)
{
    ...
    while(n){
        ...
        } else if (lt == C_CONVOLUTIONAL) {
			l = parse_c_convolutional(options, params);
...
void save_c_convolutional_weights(layer l, FILE *fp)
{
    if(l.binary){
        //save_convolutional_weights_binary(l, fp);
        //return;
...
void save_weights_upto(network net, char *filename, int cutoff)
{
    ...
    for(i = 0; i < net.n && i < cutoff; ++i){
        ...
        } else if (l.type == C_CONVOLUTIONAL) {
			save_c_convolutional_weights(l, fp);
		} if(l.type == CONNECTED){
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    for(i = start; i < net->n && i < cutoff; ++i){
        ...
        if(l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL || l.type == C_CONVOLUTIONAL){
