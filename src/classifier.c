...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    *net.iter_ = 0;
    ...
    args.d = &buffer;
    printf("before loading data 1\n");
    load_thread = load_data(args);
	printf("after loading data 1\n");

    int epoch = (*net.seen)/N;
    printf("epoch = %d\n", epoch);
    int curr_batch = get_current_batch(net);
    printf("curr_batch = %d, max_batches = %d\n", curr_batch, net.max_batches);
    while(get_current_batch(net) < net.max_batches || net.max_batches == 0){
        ...
        printf("%d, %.3f: %f loss, %f avg loss, %f rate, %lf seconds, %d images\n", get_current_batch(net), (float)(*net.seen)/N, loss, avg_loss, get_current_rate(net), sec(clock()-time), *net.seen);
        free_data(train);
        if(*net.seen/N > epoch){
            epoch = *net.seen/N;
            char buff[256];
